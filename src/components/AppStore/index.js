...
class AppStore extends Component {
  state = {activeTab: tabsList[0], text: ''}
  ...
  fun = tabId => {
    const z = tabsList.filter(ev => ev.tabId === tabId)
    this.setState({activeTab: z})
  }

  fun1 = appId => {
    const y = appsList.filter(eve => eve.appId === appId)
    this.setState({text: y.appName})
  }

  render() {
    const {activeTab, text} = this.state
    const fil = appsList.filter(each => each.category === activeTab.tabId)
    const fil1 = fil.filter(each => each.appName.includes(text))
    return (
      <div>
        <h1>App Store</h1>
        <div>
        ...
        <ul>
          {tabsList.map(every => (
            <TabItem fun={this.fun} details1={every} key={every.tabId} />
          ))}
        </ul>
        <ul>
          {fil1.map(ever => (
            <AppItem fun1={this.fun1} details2={ever} key={ever.appId} />
          ))}
        </ul>
      </div>
    )
